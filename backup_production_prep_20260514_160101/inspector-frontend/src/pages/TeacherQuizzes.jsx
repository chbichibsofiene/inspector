    ...
    return (
      <div className="quiz-session-container">
        <div className="quiz-header card">
          <button className="back-link" onClick={() => { setActiveQuiz(null); loadQuizzes(); }}>← {t('backToQuizzes')}</button>
        ...
        {submitted ? (
          <div className="evaluation-result card animate-in" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
            ...
            <h2 style={{ margin: '0 0 0.75rem', color: '#1e293b' }}>{t('quizSubmitted')}</h2>
          ...
          <form onSubmit={handleSubmit} className="quiz-form card animate-in">
            {activeQuiz.questions.map((q, idx) => (
              <div key={q.id} className="quiz-question-item">
                <div className="q-head">{t('question')} {idx + 1}</div>
            ...
            <div className="form-actions" style={{ marginTop: '2rem' }}>
              <button type="submit" className="primary-link-button" disabled={submitting}>
                {submitting ? <><Loader2 className="spinner" size={16} /> {t('submittingQuiz')}</> : t('submitQuiz')}
              </button>
            </div>
          </form>
        )}
  ...
  return (
    <div className="teacher-quizzes-page">
      <header className="page-header">
        <div>
          <h1 className="page-title">{t('assignedQuizzes')}</h1>
          <p className="page-subtitle">{t('assignedQuizzesDesc')}</p>
        </div>
      </header>
            ...
            <div key={quiz.id} className="quiz-card card">
              <div className="quiz-info">
                ...
                <div className="quiz-meta">
                  <span className="meta-tag"><BookOpen size={14} /> {t('questionsCount', { count: quiz.questions.length })}</span>
                  <span className="meta-tag"><Clock size={14} /> {t('estimatedTime')}</span>
                </div>
              </div>
              <button className="primary-link-button" onClick={() => startQuiz(quiz)}>
                {t('takeQuiz')} <ChevronRight size={16} />
              </button>
            </div>
          ))}
