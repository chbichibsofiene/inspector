  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", color: "#64748b", marginBottom: "4px" }}>
        <span>{t('lessonsCount', { count: `${value}/${total}` })}</span>
...
export default function TeacherCourses() {
  ...
  const LESSON_LABELS = { VIDEO: t('Video'), PDF: t('PDF'), QUIZ: t('Quiz') };
  ...
  const handleMarkComplete = async (lessonId) => {
    ...
    try {
      ...
      setSuccess(t('lessonCompletedEmoji'));
      ...
      <header className="page-header">
        <div>
          <h1 className="page-title">
            {view === "list" ? t('myCourses') : activeCourse?.title || t('Course')}
          </h1>
          <p className="page-subtitle">
            {view === "list"
              ? t('assignedLearningPaths')
              : t('subjectBy', { subject: activeCourse?.subject || "", inspector: activeCourse?.inspectorName || "" })}
          </p>
        </div>
        {view === "detail" && (
        ...
        <>
          {loading ? (
            ...
            <div className="card" style={{ textAlign: "center", padding: "4rem" }}>
              ...
              <h3 style={{ color: "#475569", marginBottom: "0.5rem" }}>{t('noCoursesAssigned')}</h3>
              <p className="muted">{t('noCoursesAssignedDesc')}</p>
            </div>
          ) : (
                  ...
                  >
                    {done && (
                      <div style={{
                        ...
                        <Star size={11} fill="white" /> {t('completed')}
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", gap: "1.25rem", marginBottom: "1rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "0.8rem", color: "#94a3b8" }}>
                        <Layers size={13} /> {t('modulesCount', { count: c.totalModules })}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "0.8rem", color: "#94a3b8" }}>
                        <BookOpen size={13} /> {t('lessonsCount', { count: c.totalLessons })}
                      </div>
                    </div>

                    <ProgressBar value={c.completedLessons ?? 0} total={c.totalLessons ?? 0} t={t} />
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "1rem" }}>
                      <span style={{
                        ...
                        {done ? t('review') : t('continue')} <ChevronRight size={14} />
                      </span>
                    </div>
                  </div>
                );
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: "1.5rem" }}>
            <div className="card-header" style={{ marginBottom: "1rem" }}>
              <div className="card-title">{t('yourProgress')}</div>
              <span style={{
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, color: "#1e293b", fontSize: "0.95rem" }}>
                      {t('module')} {mi + 1}: {mod.title}
                    </div>
                    <div style={{ fontSize: "0.78rem", color: "#94a3b8", marginTop: "2px" }}>
                      {t('lessonsCompleted', { completed: modCompleted, total: modTotal })}
                    </div>
                  </div>
                          ...
                          <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                            {les.contentUrl && (
                              <button 
                                ...
                                <PlayCircle size={14} /> {les.type === 'VIDEO' ? t('watch') : t('read')}
                              </button>
                            )}
                            {!les.completed ? (
                              <button
                              ...
                              >
                                {isMarking
                                  ? <><Loader2 className="spinner" size={13} /> {t('saving')}</>
                                  : <><CheckCircle size={13} /> {t('markDone')}</>}
                              </button>
                            ) : (
                              <span style={{
          ...
          {activeCourse.progressPercent === 100 && (
            <div className="card animate-in" style={{
              ...
              <h3 style={{ margin: "0 0 0.5rem", color: "#065f46" }}>{t('courseCompleted')}</h3>
                  ...
                  {quizLoading ? (
                    ...
                    <form onSubmit={handleQuizSubmit} className="quiz-viewer-form animate-in">
                      <div className="quiz-info-bar">
                        ...
                        <div className="quiz-q-count">{t('questionsCount', { count: quizData.questions?.length })}</div>
                      </div>
                      
                      {quizData.questions?.map((q, idx) => (
                        <div key={q.id} className="quiz-viewer-q">
                          <div className="q-index">{t('question')} {idx + 1}</div>
                          ...
                          {q.type === 'MCQ' ? (
                            ...
                            <textarea 
                              ...
                              placeholder={t('typePedagogicalResponse')}
                      ...
                      <div className="quiz-viewer-actions">
                        <button type="submit" className="primary-link-button" disabled={quizSubmitting}>
                          {quizSubmitting ? <><Loader2 className="spinner" size={16} /> {t('Evaluating')}...</> : t('submitQuiz')}
                        </button>
                      </div>
                    </form>
                  ) : (
                    <div className="quiz-viewer-placeholder">
                      <Brain size={48} />
                      <h3>{t('integratedQuizSystem')}</h3>
                      <p>{t('lessonLinkedQuiz')}</p>
                    </div>
                  )}
                </div>
              ) : (
                <iframe src={selectedLesson.contentUrl} className="pdf-viewer" title={selectedLesson.title} />
              )}
            </div>
