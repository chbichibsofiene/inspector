  ...
  return (
    <div className="admin-page">
      <header className="page-header">
        <div>
          <h1 className="page-title">{t('userActivityHistory')}</h1>
          <p className="page-subtitle">{t('trackTimeline')}</p>
        </div>
      </header>
      ...
      <section className="card" style={{ marginBottom: '1.5rem' }}>
        <form onSubmit={(e) => { e.preventDefault(); performSearch(); }} style={{ display: 'flex', gap: '1rem' }}>
          <input 
            type="text" 
            placeholder={t('enterUserIdentifier')}
            value={userId} 
          ...
          <button type="submit" className="btn-primary" disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Search size={18} />
            {loading ? t('searching') : t('trackHistory')}
          </button>
        </form>

        {suggestedUsers.length > 0 && history.length === 0 && (
          <div style={{ marginTop: '1.5rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#64748b', fontWeight: 600, marginBottom: '0.75rem' }}>{t('recentlyRegisteredUsers')}</p>
      ...
      <section className="timeline-container">
        {history.length > 0 ? (
          ...
          !loading && <div className="empty-state card">{t('noHistoryFound')}</div>
        )}
      </section>
    </div>
  );
