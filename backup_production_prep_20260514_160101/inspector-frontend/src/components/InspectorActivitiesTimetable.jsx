...
export default function InspectorActivitiesTimetable() {
  ...
  const loadActivities = async () => {
    try {
      ...
      setError(t('unableToLoadActivities'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="card timetable-card" style={{ marginTop: "2rem" }}>
      <div className="card-header">
        <div>
          <div className="card-title">{t('inspectorActivitiesTimetable')}</div>
          <div className="card-subtitle">{t('eventsInvitedByInspector')}</div>
        </div>
            ...
            <div key={day} className="timetable-day-column">
              ...
              <div className="day-slots">
                {grouped[day].length === 0 ? (
                  <div className="empty-day">{t('free')}</div>
                ) : (
                    ...
                    return (
                      <div key={activity.id} className="slot-item inspector-slot" style={{
                        ...
                        <div className="slot-footer" style={{ marginTop: "6px", color: "#94a3b8" }}>
                          <MapPin size={11} /> {activity.location || t('onlineTbd')}
                        </div>
      ...
      {!loading && activities.length === 0 && (
        <div style={{ textAlign: "center", padding: "3rem 2rem", color: "#94a3b8" }}>
          ...
          <h3 style={{ margin: "0 0 0.5rem" }}>{t('noAssignedActivities')}</h3>
          <p>{t('noAssignedActivitiesDesc')}</p>
        </div>
      )}
