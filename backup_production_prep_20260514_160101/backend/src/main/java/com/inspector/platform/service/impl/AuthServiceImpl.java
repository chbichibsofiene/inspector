...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        try {
            ...
            if (user.getRole() == Role.INSPECTOR) {
                var profile = inspectorProfileRepository.findByUserId(user.getId());
                if (profile.isPresent()) {
                    firstName = profile.get().getFirstName();
                    lastName = profile.get().getLastName();
                }
            } else if (user.getRole() == Role.TEACHER) {
                var profile = teacherProfileRepository.findByUserId(user.getId());
                if (profile.isPresent()) {
                    firstName = profile.get().getFirstName();
                    lastName = profile.get().getLastName();
                }
    ...
    public void forgotPassword(String email) {
        ...
        logService.log(user, com.inspector.platform.entity.ActionType.UPDATE, "User", user.getId().toString(), "Password reset code requested", null);
