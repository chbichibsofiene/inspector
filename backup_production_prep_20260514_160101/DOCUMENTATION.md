...
The Inspector Platform can be launched using either Docker for a one-click setup or manually for development purposes.

### 📋 Prerequisites
- **Java 17 (JDK)**: Required for the Spring Boot backend.
- **Node.js 18+**: Required for the React and React Native frontends.
- **XAMPP / MySQL Server**: (Optional) Required if you wish to use a persistent local database.
- **Expo Go**: (Optional) For testing the mobile app on a physical device.

---

### 🚀 Option A: Docker (Recommended)
This is the fastest way to get the entire ecosystem running in a consistent environment.
...
If using MySQL (via XAMPP or native):
1. Start **MySQL** via XAMPP Control Panel.
2. Create a database named `inspector_system_db`.
...
#### 4. Start the Mobile Frontend (Expo)
```powershell
cd mobile-frontend
npm install
npx expo start
...
> **Note for mobile**: Create a `.env` file from `.env.example` and set `EXPO_PUBLIC_API_BASE_URL` to point to your running backend (e.g. `http://10.0.2.2:8081` for Android emulator).
...
| Service | Command |
| :--- | :--- |
| Backend (MySQL) | `cd backend && .\apache-maven-3.9.6\bin\mvn.cmd spring-boot:run` |
| Backend (H2 local) | `cd backend && .\apache-maven-3.9.6\bin\mvn.cmd spring-boot:run "-Dspring-boot.run.profiles=local"` |
| Frontend Web | `cd inspector-frontend && npm install && npm run dev` |
| Frontend Mobile | `cd mobile-frontend && npm install && npx expo start` |
